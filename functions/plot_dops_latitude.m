function plot_dops_latitude(number_satellites, altitudes, ...
    latGridInDegrees, latBins,...
    data_map_walker, data_map_core_constellation, percentile,...
    legend_pos)
%% PLOT CDF OF DOPS
%
...
linestyle_plot_leo = {'-','-',':','-.'};
linewidth_plot_leo = [3,3,3,3]*2.5/3;
...
for k = 1:8
   if k <=4
       ...
       plot(100000,1000000, ...
           [linestyle_plot_leo{k-4}, marker_plot_leo{k-4}], ...
           'color', colors_leo(k-4,:), ...
           'LineWidth', linewidth_plot_leo(k-4), ...
           'MarkerFaceColor',colors_leo(k-4,:), ...
           'MarkerSize', markersize_plot_leo(k-4))
   end % end if  
end % end k 
...
h = legend('GPS','Galileo','GLONASS','BeiDou',...
    'Iridium','Teledesic','OneWeb','SpaceX');
legend boxoff
